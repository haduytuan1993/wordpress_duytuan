<template>
	<div class="quiz-composer quiz-text-question">
		<v-container fluid grid-list-lg>
			<v-layout row wrap>
				...
				<v-flex sm8>
					<v-radio-group v-model="rightQuestion" v-on:change="setRightQuestion" column>

						<template v-for="(q, index) in questions">
							<div class="item">
								<v-layout row wrap class="white mr-0 ml-0">
									<v-flex sm1>
										<v-radio :value="q.id"></v-radio>
									</v-flex>
									<v-flex sm11>
										<textarea v-model="q.content"></textarea>
									</v-flex>
								</v-layout>
					...
					<v-btn v-if="questions.length < 3" small round primary class="mt-3" @click="addQuestion">Add</v-btn>
				</v-flex>
				<v-flex sm4>
...
export default {
	...
	data: function(){
		...
		if( typeof this.question.image == 'undefined' ) {

			this.question.image = 0;
		}
		...
		if( typeof this.question.options == 'undefined' ) {
			...
			this.question.options = [
						{ id: 1, content: 'Question 1', is_correct: 1 },
						{ id: 2, content: 'Question 2', is_correct: 0 },
						{ id: 3, content: 'Question 3', is_correct: 0 }
					];
		}
	...
	mounted: function(){
		...
		if( typeof this.question.options != 'undefined' && this.question.options != null && this.question.options.length > 0) {
